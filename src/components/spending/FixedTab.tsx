        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-ink-500">
              {pct != null
                ? `월 소득 대비 ${pct.toFixed(0)}% · 결제일은 본인이 세팅`
                : "진단에 소득을 넣으면 비중을 계산해요"}
            </p>
          </div>
          <p className="max-w-xs text-xs leading-relaxed text-ink-400">
            고정은 레버가 아닙니다. 여기서는 목록만 정리하고, 조절은 변동지출에서 하세요.
          </p>
        </div>
      </Card>
              ...
              <tr key={f.id} className="border-b border-ink-50 last:border-0">
                ...
                <td className="px-3 py-2">
                  <select
                    ...
                    value={f.category}
                    onChange={(e) =>
                      updateFixed(f.id, { category: e.target.value as SpendCategory })
                    }
                  >
