...
export function NumberInput({
  ...
  return (
    <div className="flex items-center rounded-xl border border-slate-300 bg-white focus-within:border-brand-500 focus-within:ring-1 focus-within:ring-brand-500">
      <input
        inputMode="numeric"
        className="w-full rounded-xl bg-transparent px-3 py-2 text-sm text-slate-900 outline-none"
...
export function TextInput({
  ...
  return (
    <input
      className={clsx(
        "w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500",
        className,
      )}
      value={value}
...
export function AssumptionNote({ children }: { children?: ReactNode }) {
  return (
    <p className="text-xs text-slate-400">
      ⚠️ {children ?? "모든 수치는 예시·가정이며 수익을 보장하지 않습니다. 개별 종목·매물 추천이 아닌 '배분 구조'만 다룹니다."}
    </p>
  );
}
