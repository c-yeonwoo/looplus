...
작은 실행을 반복해 여러 개의 큰 자산·인생 루프를 하나씩 완성하는 플랫폼.

> **Loop+** (영문 Looplus / 한글 루플러스) — 자산·현금흐름이 루프로 돌고, 돌수록 더해진다.
> "정답을 강요하지 않고, 내가 내 엔진과 작은 루프를 만들도록 곁에서 돕는다."
...
**온보딩(첫 숫자·첫 큰 루프) → 재테크 엔진 → 홈 대시보드 → 작은 루프 실행.**
핵심 루프 = 큰 목표 설정 → 현재 입력 → 엔진 구성 → 미래 결과 확인 → 이번 주 실행 → 변화 모니터링.

- **큰 루프:** 순자산·비상금·저축률·월 현금흐름·사용자 정의 목표처럼 완료를 확인할 수 있는 장기 목표.
- **작은 루프:** 큰 루프를 밀어 주는 주간/일간 실행. 완료 기록과 연결해 "무엇을 반복하는지"를 잃지 않는다.
- 계정 없이도 첫 곡선과 루프를 체험할 수 있으며, 가입하면 이 기기의 데이터를 계정에 백업한다.

제외(이후): AI 재무 피드백(G) · 커뮤니티(백로그) ·
오픈뱅킹/마이데이터 자동연동(2차).

비전보드 장면 사진은 Gemini 로 생성한다 (`GEMINI_API_KEY` 설정 시에만 노출).
...
src/
  app/
    (app)/            # 사이드바/하단탭 쉘이 감싸는 메인 앱
      home/           # 홈 대시보드 (달성률·ETA·지금 돌리는 루프)
      engine/         # C 재테크 엔진 (핵심)
      diagnosis/      # B 진단
      goals/          # A 큰 루프·목표·비전
      spending/       # E 지출관리 (실측·예산·엔진 연동)
    onboarding/       # A→B→C 위저드 (쉘 없음)
    page.tsx          # 진입점: 온보딩 여부로 라우팅
  components/
    engine/           # EngineBuilder · Palette · Inspector · EngineFlow
    panels/           # DiagnosisPanel · GoalsPanel · TrackingPanel (페이지·온보딩 공용)
    loops/            # 큰 루프 생성·진행률·작은 루프 연결
...
| AI 장면 생성 | Gemini · `GEMINI_API_KEY` 있을 때만 노출 | 768px webp 로 줄여 저장 (원본은 장당 1MB+) |
| 넛지 임계값 | ETA > 목표 시점 × **1.5** 또는 시점 내 미도달 | `needsRealityNudge` |
| nav 형태 | **웹 사이드바 + 모바일 하단탭** | |
| 보유 자산 | 각 투자·저축 버킷의 `currentBalance` | 캔버스에서 흐름(월) + 보유를 한눈에 |
| 미배분 잔여 | 진단 총액 − 버킷 합이 있으면 잔여로 굴림 | 부분 입력·마이그레이션 대비 |
| 자산발 자본소득 | `max(진단 입력값, 버킷·잔여 실현분)` | 같은 자산에서 나온 돈이라 더하지 않는다 |
