...
| 보유 자산 | 버킷과 **분리**. 현금·투자자산·부동산 각각의 수익 가정으로 굴린다 (`engine/holdings.ts`) | 흐름 비율로 쪼개면 실제 보유 구성과 무관해짐 |
| 버킷 시작 잔액 | **0** — 배분받은 흐름만 쌓인다 | year 0 순자산 = 현재 순자산 보장 |
| 자산발 자본소득 | `max(진단 입력값, 보유 자산 × 실현률)` | 같은 자산에서 나온 돈이라 더하지 않는다 |
